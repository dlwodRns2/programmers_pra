package org.example;
//* 추상 클래스
//추상 클래스는 인스턴스화 할 수 없는 클래스임
//즉, 추상 클래스 자체로 객체를 생성할 수 없으며, 이 클래스를 상속받은 자식 클래스를 통해서만 객체를 생성할 수 있다.
//추상 클래스는 추상 메서드 뿐만 아니라 일반 메서드도 포함될 수 있으며, 필드(멤버 변수를 가질 수도 있다.

//* 추상 메서드
//추상 메서드는 메서드의 선언만 존재하고, 구현(메서드 몸체)이 없는 메서드이다.
...
//이 경우 인터페이스를 사용해야 하지만, 인터페이스는 구현을 가질 수 없다는 제약이 있다.
//2. 구현의 강제성 - 모든 자식 클래스에서 구현 강제
//추상 메서드를 상속받는 모든 자식 클래스는 해당 메서드를 반드시 구현해야한다.
...
//- 구현 상속의 어려움
//공통된 코드가 적절히 분리되지 않으면 추상 클래스의 유지보수가 어려워질 수 있다.
