...
//* 인터페이스
//자바에서 클래스들이 구현해야 하는 메서드의 집합을 정의하는데 사용되는 추상 타입.
//인터페이스는 메서드의 선언만 포함하며, 메서드의 구현은 포함하지 않는다.
//따라서 인터페이스를 구현하는 클래스는 인터페이스의 모든 메서드를 구현해야 한다.

//* 인터페이스의 필요성
//1. 표준화 : 인터페이스를 사용하면 여러 클래스가 동시에 동일한 메서드를 구현하도록 강제할 수 있다.
//이는 일관된 API 설계와 코드 표준화를 가능하게 한다.

//2. 다중 상속의 대안 : 자바는 클래스의 다중 상속을 지원하지 않지만, 인터페이스는 여러개를 구현 가능.
//이를 통해 다중 상속의 이점을 제공하면서, 다중 상속에서 발생할 수 있는 복잡성과 충돌을 피할 수 있다.

//3. 유연한 설계(중요) : 인터페이스를 사용하면 특정 클래스의 구현에 의존하지 않고, 인터페이스를 기반으로 프로그래밍할 수 있다.
//이는 코드의 유연성과 확장성을 크게 향상시킨다.

//4. 느슨한 결합 : 시스템의 각 부분을 독립적으로 개발하고 변경할 수 있게 한다.
