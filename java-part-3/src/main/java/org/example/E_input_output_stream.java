...
//* I/O Stream(입출력 스트림)
//자바에서 데이터를 입력받거나 출력할 때 사용하는, 데이터가 흐르는 통로이다.
//스트림은 단방향이라서 입력용(InputStream), 출력용(OutputStream)이 따로 존재한다

//* 바이트 스트림 vs 문자 스트림
//바이트 스트림 : 1바이트 단위로 데이터를 처리한다 => 이미지/동영상 등 모든 종류의 데이터는 바이트 스트림
//문자 스트림 : 2바이트(문자) 단위로 데이터를 저장한다 -> 텍스트, 문자 스트림이 적합
//Writer, Reader => 스트림 사용 후, 반드시  close로 자원을 해제해야 한다 -> (try-with-resources)
public class E_input_output_stream {
    ...
    public E_input_output_stream() {
        ...
        System.out.println("Desktop Path : "+this.desktopPath);
        this.folderPath = this.desktopPath + File.separator + "sample";

        // Paths.get(...)은 문자열로 된 경로를 Path 객체로 변환한다
        // Path 객체는 경로 관련된 간단한 기능만을 제공하고,
        // Path를 사용해 실제 작업을 하려면 File 클래스에 Path 객체를 넘겨주고 File 객체로 작업을 실행한다.
    ...
    //2. 파일에 내용 쓰기
    //	•	FileOutputStream으로 파일에 바이트 데이터를 기록.
    ...
    public void exam2(){
        ...
        if(Files.notExists(todayFile)){
            //방법1 : 전통적인 방식(직접 close)
            //finally 블록에서 스트림을 직접 닫아주어야 하므로 코드가 길고 복잡함
            ...
            //방법2 : try-with-resources
            //try()내에 선언한 자원은 try 블록이 끝나면 자동으로 close가 호출됨
            //따라서 finally로 직접 닫을 필요가 없어 코드가 간결하고 안전
    ...
    //	•	QR코드는 이미지(PNG)이므로 문자가 아닌 "바이너리 데이터"이다. -> 바이트 스트림이 적합하다.
    //	•	exam2()처럼 텍스트가 아니라 이미지라서, 문자 스트림으로는 다룰 수 없는 대표 예시다.
    //	•	ZXing 라이브러리로 문자열을 QR코드 행렬(BitMatrix)로 만든 후 PNG파일로 저장한다.
    public void exam2_1(){
        today = LocalDate.now().format(DateTimeFormatter.ofPattern("yyyy-mm-dd"));
    ...
    //2-2. 파일에 내용 쓰기 - 문자
    //	•	exam2()는 바이트 스트림(FileOutputStream)이라 문자열을 getBytes()로 byte 배열로 바꿔서 써야 했음.
    ...
    public void exam3_2(){
        ...
        if(Files.exists(todayFile)){
            ...
            //FileReader(문자 스트림)을 BufferdReader로 감싸, 줄 단위로 읽는다.
