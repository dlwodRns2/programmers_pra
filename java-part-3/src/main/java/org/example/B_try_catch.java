...
//* 예외처리
//프로그램 실행 중에 발생할 수 있는 오류(예외)를 처리하여, 프로그램이 갑자기 멈추기 않도록 하는 방법
...
public class B_try_catch {
    ...
    public static void exam2(){
        try{
            ...
            System.out.println("배열 인덱스 예외 처리"+e);
        }catch(Exception e){
            System.out.println("그 외 예외 처리 : "+e);
        }
    }
    //2-2. 멀티 catch
    //처리 방식이 같은 여러 예외는 | 파이프로 묶어 하나의 catch에서 처리할 수 있다(java 7 이상)
    ...
    //try-with-resource를 사용하지 않는 경우 : finally문에서 fis.close 부분을 개발자가 직접 작성해야함
