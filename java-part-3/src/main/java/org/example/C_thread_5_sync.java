...
//- 왜 동기화가 필요할까?
//쓰레드가 1개면 자원을 혼자 쓰니 문제가 없음
//쓰레드가 여러 개면 같은 자원을 함꼐 쓰는데, 동시에 건드리면 값이 꼬일 수 있다.

//- 임계 영역(Critical section) : 한 번에 한 쓰레드만 들어가야만 하는 코드 영역
//- 잠금(lock) : 그 구역에 들어가려면 받아야 하는 열쇠
//-> 열쇠를 가진 한 쓰레드만 들어가고, 일을 끝내고 열쇠를 반납해야 다음 쓰레드가 들어갈 수 있다.

//이렇게 한 쓰레드의 작업을 다른 쓰레드가 방해하지 못하게 막는 것을 '동기화' 라고 한다.

//* 뮤텍스(상호 배제)
//열쇠는 단 1개 -> 한 번에 오직 한 쓰레드만 임계 영역에 들어갈 수 있게 하는 잠금
...
//-> lock을 걸고 있으면 다른 쓰레드는 기다려야 해서 성능 저하
...
public class C_thread_5_sync {
    static void main(String[] args) {
        ...
        new Thread(r).start(); //출금 쓰레드2
        new Thread(r).start(); //출금 쓰레드1
    }
