...
//record란? (java 16+) : 필드 나열만 하면 나머지 상용구를 컴파일러가 다 작성해주는 불변 데이터 전용 클래스
//필드선언, 생성자, getter, equals/hashCode/toString 까지 수십 줄을 대신 해줌

//record 사용하는 이유 : 상태는 응답 맵 하나뿐인 불변 값 객체.
//컴포넌트 이름을 attributes로 지으면 인터페이스 attributes()가 자동 구현됨.
