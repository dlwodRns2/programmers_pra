...
//여기까지 오면 "SNS 인증"은 이미 끝난 상태임(state 검증/토큰 교환/user-info 조회 완료)
//그런데 우리는 세션을 쓰지 않으므로(STATELESS), SNS 인증 결과를 "우리 서비스의 신분증"인
//자체 JWT(access/refresh)로 바꿔서 클라이언트에게 건네주는 것이 이 클래스의 역할.

//왜 SNS가 준 토큰을 그대로 안 쓰고 우리 JWT 토큰을 새로 발급하는가
