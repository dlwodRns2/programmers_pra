...
import org.example.oauth2.config.oauth2.CustomOAuth2User;
import org.example.oauth2.config.oauth2.OAuth2UserInfo;
import org.example.oauth2.config.oauth2.OAuth2UserInfoFactory;
...
//* CustomOAuth2UserService - OAuth2 파이프라인에서 개발자가 구현하는 첫 번째 훅
...
//사용자가 동의해야 그 때 비스로소 계정이 생성됨.(이메일이 같아도 제공자가 다르면 별개 계정 -> 식별 기준 : provider + providerId)

//* DefaultOAuth2UserService
//부모의 loadUser()가 "제공자 user-info 엔드포인트로 HTTP 요청을 보내 프로필을 받아오는"
//코드 전부를 이미 갖고 있다. 통신을 다시 구현할 이유가 없으므로 super.loadUser()로 결과만 받고,
//그 뒤의 "우리 도메인 연결"만 덧붙인다.
@Service
...
public class CustomOAuth2UserService extends DefaultOAuth2UserService {
    ...
    public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
        ...
        //CustomOAuth2User.getName()이 이 키로 식별자를 꺼낼 때 사용.
        ...
        //loadUser()는 시큐리티 필터(OAuth2LoginAuthenticationFilter) 안에서 호출된다.
