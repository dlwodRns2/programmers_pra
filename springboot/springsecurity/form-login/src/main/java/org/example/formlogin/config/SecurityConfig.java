...
//* 폼 로그인이란?
//개발자가 직접 만든 HTML 로그인 화면(폼)을 통해 아이디/비밀번호를 받아 인증하는 방식.
//HTTP BASIC이 브라우저 기본 팝업 + 헤더 방식이었다면,
//폼 로그인은 우리가 디자인한 페이지 + 세션 기반이라는 점이 가장 큰 차이
...
//이 처리는 AuthenticationEntryPoint(폼 로그인 rn현체 LonginUrlAuthenticationEntryPoint)가 담당
//2. 사용자가 폼에 입력하고 제출
...
//그리고 서버는 세션ID를 JSESSIONID라는 쿠키로 브라우저에 내려준다
//성공 시 처리는 AuthenticationSuccessHandler가 담당(기본적으로 원래 가려던 페이지 또는 지정된 페이지로 리다이렉트)
//실패 시 처리는 AuthenticationFailureHandler가 담당(보통 /login?error로 되돌림)
//6. 이후 요청 -> 쿠키로 인증 유지
...
//로그인처럼 "너는 누구냐"를 확인하는 과정

//* 인가(Authorization) 흐름
//"인증된 사용자가 이 자원에 접근할 권한이 있냐"를 판단하는 과정
...
public class SecurityConfig {
    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception{
        http
                ...
                //로그인된 사용자의 브라우저를 속여, 사용자가 의도하지 않는 요청을 우리 서버로 보내게 만드는 공격
                ...
                //인가 규칙 - URL별 접근 제어. AuthorizationFilter가 이 규칙대로 판단.
                ...
                .logout(
                    logout -> logout
                            //d이 URL로 오는 요청을 LogoutFilter가 가로챈다
