...
//* 프레젠테이션 개종 테스트 - 컨트롤러의 "Http 계약을 검증한다"

//무엇을 보나?
//URL 매핑, 요청 본문(JSON) 파싱, 상태 코드, 응답 JSON => 상태 코드 변환
//"비즈니스 로직"이 아니라, "웹 껍데기가 제대로 동작하는가" 가 관심사
@WebMvcTest(MemberApiController.class)
class MemberApiControllerTest {

    //MockMvc : 실제 서버(톰캣)을 띄우지 않고, Http 요청을 "흉내내사" 컨트롤러에 넣어보는 도구
    @Autowired
    private MockMvc mockMvc;

    //ObjectMapper : 객체를 JSON으로 변환할 때 사용
    @Autowired
    ...
    void join_성공() throws Exception {
        ...
        //mockMvc.perform(요청 생성).andExpect(기대 검증).andExpect(기대 검증) ...
        //- perform(요청 생성) : 요청을 "실행"한다. 실제 톰캣 없이 스프링MVC 내부로 요청을 흘려보낸다
        //- andExpect(기대 검증) : 그 결과(상태코드/헤더/본문)가 기대에 맞는지 검증한다.
    ...
    @Test
    @DisplayName("회원가입 실패 - 아이디가 중복이면 409와 에러 메시지를 반환")
