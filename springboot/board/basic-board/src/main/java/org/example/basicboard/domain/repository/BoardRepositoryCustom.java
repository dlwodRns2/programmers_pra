...
// 우리가 직접 코드를 넣을 수 없음
//- 그래서 "직접 구현할 메서드"는 이 별도 인터페이스에 선언하고, 실제 코드는 BoardRepositoryImpl에 작성한다.
//- 그리고 BoardRepository가 이 인터페이스를 함께 상속하면(extends)
//Spring Data가 "자동 생성 메서드 + 우리가 짠 메서드"를 하나의 레포지토리로 합쳐준다.

//* 이름 규칙이 중요 : 구현 클래스는 반드시 "<레포지토리 이름> + Impl이어야 한다.
//- BoardRepositoryCustom의 구현체 이름은 BoardRepositoryCustomImpl로 맞춰야 스프링이 잗동으로 연결함
