package org.example.basicboard.scheduler;
...
//* @Scheduled 메서드의 형태 : 파라미터 X + 반환타입 void
...
public class BoardBatchScheduler {
    ...
    //      │   └──────────────── 분   (0~59) → 0분
    //      └──────────────────── 초   (0~59) → 0초
    // 스프링 cron은 "6자리" 이다. (맨 앞이 second)
    // linux crontab은 5자리이다(minute 부터 시작)
    // crontab 예제를 그대로 스프링에 적용하면 한 칸씩 밀려 엉뚱한 시각에 메서드가 돌아감
    ...
    // Zone 옵션 : 시각의 기준 시간대를 지정한다.
    //- 지정하지 않으면 "서버(JVM)의 시간대"를 따라감
    //=> 해외 클라우드 서버(UTC)에 배포하면, "9시에 돌아라"가 한국 시간 18시에 도는 사고가 발샘
    ...
    public void reportOrphanFiles(){
        ...
        //DB가 참조중인 파일 이름 집합을 만든다(경로 형태가 제각각일 수 있어 "파일 이름"으로 비교)
        //List가 아니라 Set인 이유 : contains 검사가 목록 크가와 무관하게 빠르다(파일 수 * 글 수 반복을 피함)
        Stream<String> stringStream = boardRepository.findAll().stream()
                .map(Board::getFilePath);

        Set<String> reference = boardRepository.findAll().stream()
                .map(Board::getFilePath)
        ...
        int orphanCount = 0;
        for(File file : files){
            if(!reference.contains(file.getName())){
                orphanCount++;
