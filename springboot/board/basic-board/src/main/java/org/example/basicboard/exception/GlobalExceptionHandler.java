...
//"모든 컨트롤러에 공통으로 적용되는 보조 클래스" 임을 선언하는 애너테이션.
//특정 컨트롤러 한 개가 아니라, 애플리케이션의 "모든 @Controller/@RestController"에서 발생하는 예외를 가로챈다

//* 전역 예외 처리
//예외가 터질 때마다 컨트롤러 안에서 try-catch로 일일이 잡으면, 컨트롤러마다 같은 코드가 반복됨
//핵심 로직과 예외 처리 코드가 뒤섞여서 지저분해지고, 응답 형태(상태코드/메시지)도 제각각이 되기 쉬움
//그래서 "예외 처리"라는 공통 관심사를 한 곳에 모아두고, 컨트롤러/서비스는 예외를 "던지기만" 하게 만든다.
//-> 컨트롤러는 성공 흐름(정상 로직)에만 집중하고, 예외 -> 응답 변환은 전부 이 클래스가 책임진다.

//* 전체 흐름
//서비스 : throw new DuplicationUserIdException("이미 존재하는 아이디입니다.")
//-> 컨트롤러는 에외를 잡지 않고 그대로 위로 전파됨
//  -> 여기 GlobalExceptionHandler가 가로챔
//      -> 상태코드(409) + ErrorResponseDto(JSON)로 변환해 응답
//          -> signUp.js의 error 콜백이 message를 꺼내 화면에 표시


//* 그런데 예외 처리도 "공통 관심사" 인데, 왜 AOP가 아니라 @RestControllerAdvice를 사용하는가?
//사실 @RestControllerAdvice도 내부적으로 AOP와 같은 "공통 관심사 분리" 아이디어 위에 서있음.
//"범용 AOP를 직접 만들 것이냐 vs 예외 처리에 특화된 전용 도구를 사용할 것이냐" 의 문제임.

//1) AOP는 "컨트롤러 메서드 호출" 그 순간만 감쌀 수 있음.
//- @Around는 컨트롤러 메서드가 실제로 실행되는 지점만 try-catch로 감싼다
//- 하지만 웹 예외는 메서드 "바깥"에서도 터짐(예 : @Valid 검증 실패, JSON <-> DTO 변환 실패, 파라미터 타입 불일치 등)
//이런 예외는 컨트롤러 메서드가 호출되기도 "전"에 발생해서 AOP 포인트 컷에 잡히지 않음
//- @RestControllerAdvice는 스프링 MVC 예외 처리 파이프라인에 연결되어,이런 프레임워크 단계의 예외까지 잡아준다.

//2) 응답 만들기를 프레임워크가 대신 해준다
//- AOP로 직접하면 예외를 잡은 뒤, 상태코드 세팅, JSON 직렬화, Content-type 형성을 전부 직접 손으로 작성해야함
//- @RestControllerAdvice는 ResponseEntity / 메시지 컨버터(DTO 자동 JSON 변환) / 예외 타입 매핑을 이미 제공

//3) 예외 타입별 분기가 선언적이다
//- @ExceptionHandler(타입)으로 "이 예외는 이 메서드가 처리"를 애너테이션으로 나눈다.
//- AOP로 하면, if(e instance of ...) 분기를 직접 나열해야 해서 읽기 어렵고 유지보수가 나쁘다

//# 한 줄 요약 : 도구를 목적에 맞게
// - 로깅/실행시간 측정/트랜잭션 처럼 "무엇에나 끼워 넣는 범용 작업" => AOP(@Aspect)
