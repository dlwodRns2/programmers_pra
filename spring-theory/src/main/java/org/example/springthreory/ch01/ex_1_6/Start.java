package org.example.springthreory.ch01.ex_1_6;

import org.example.springthreory.ch01.ex_1_6.dao.DaoFactory;
import org.example.springthreory.ch01.ex_1_6.dao.UserDAO;
...
//* 싱글톤 레지스트리와 오브젝트 스코프
//Application Context는 싱글톤을 저장하고 관리하는 싱글톤 레지스트리 이기도 하다
//스프링은 기본적으로 별다른 설정읋 하지 않으면 내부적으로 빈 오브젝트를 모두 싱글톤으로 생성한다
...
//* 스프링 빈의 스코프
//스프링이 관리하는 오브젝트(빈)이 생성되고, 존재하고, 적용되는 범위를 빈의 스코프라 함
//스프링 빈의 기본 스코프는 싱글톤
//싱글톤 스코프는 컨테이너 내에 한 개의 오브적트만 만들어서, 강제로 제거하지 않는 한 스프링 컨테이너가 존재하는 동안 계속 유지
//경우에 따라서는 싱글톤 외의 스코프를 가질 수 있다
//웹을 통해 새로운 HTTP 요청이 생길 때마다 생성되는 요청 스코프가 있다.

//"요청이 몰리면 대기하는가?" 에 대한 답 : 대기 여부는 빈 스코프가 아닌 스레드 수가 결정
//
//  ┌────────────────────────┬─────────────────────────────────────────┐
//  │          구분          │              무엇이 정하나              │
//  ├────────────────────────┼─────────────────────────────────────────┤
...
public class Start {
    public static void main(String[] args) throws SQLException, ClassNotFoundException {
        ...
        UserDAO userDAO4 = context.getBean(UserDAO.class);
