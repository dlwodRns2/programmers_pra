package org.example.springthreory.ch01.ex_1_5;

import org.example.springthreory.ch01.ex_1_5.dao.DaoFactory;
import org.example.springthreory.ch01.ex_1_5.dao.UserDAO;
...
//이제 DaoFactory를 스프링에서 사용이 가능하도록 변신
//스프링에서는 스프링이 제어권을 가지고 직접 만들고 관계를 부여하는 오브젝트를 빈(Bean)이라 한다.
//자바 빈은 오브젝트 단위의 애플리케이션 컴포넌트를 의미하고
//동시에 스프링 빈은 컨테이너가 생성과 관계설정, 사용 등을 제어해주는 제어의 역전이 적용된 오브젝트를 가리킨다
//스프링에서는 빈의 생성과 관게설정 같은 제어를 담당하는 IOC 오브젝트를 빈 팩토리라 한다.
//정확히는 빈 팩토리보다는 이를 좀 더 확장한 애플리케이션 컨텍스트(빈 팩토리를 상속)를 주로 사용한다
//애플리케이션 컨텍스트는 IoC 방식을 따라 만들어진 일종의 빈 팩토리라고 생각하면 된다.

//* 컴포넌트(Component) : 애플리케이션을 구성하는, 독립적으로 같이 끼울 수 있는 부품 같은 오브젝트
...
//* 컨테이너(Container)
//컨테이너란 이 컴포넌트를 담아두고, 생성하고, 서로 연결(관계설정)하고, 생명주기까지 관리해주는 실행 환경
