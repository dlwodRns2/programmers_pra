package org.example.springthreory.ch01.ex_1_4;

import org.example.springthreory.ch01.ex_1_4.dao.DaoFactory;
import org.example.springthreory.ch01.ex_1_4.dao.UserDAO;
...
//오브젝트 팩토리
//개게의 생성 방법을 결정하고 그렇게 만들어진 오브젝트를 돌려주는 일을 하는 오브젝트를 '팩토리'라 리

//* 제어의 역전
//제어의 역전이란 오브젝트가 자신이 사용할 오브젝트를 스스로 선택 및 생성하지 않고
//또 자신이 언제 어떻게 만들어지는지 조차 모른 채 그 제어권을 외부에 넘기는 것.

//UserDAO 입장에서 두 가지 제어권이 사라짐
//1. 어떤 ConnectionMaker를 사용할지에 대한 제어권(DaoFactory가 어떤 ConnectionMaker를 줄 지 제어권을 가져감)
//  public UserDAO(SimpleConnectionMaker simpleConnectionMaker) {
//      this.simpleConnectionMaker = simpleConnectionMaker;
//  }

//2. 자기 자신의 생성에 대한 제어 : UserDAO가 언제, 어떤 의존성과 함께 만들어지는지를 DaoFactory가 결정한다.
...
//  }
//즉, "어떤 구현을 쓸 지 결정"하고 "오브젝트를 생성 및 연결"하는 제어 권한이 UserDAO에서 DaoFactory로 넘어갔다
//이것이 바로 "제어의 역전"이다.
