package org.example.springthreory.ex_6.ex_6_1;

//* AOP(Aspect Oriented Programming, 관점지향 프로그래밍)
//여러 모듈에서 공통적으로 흩어져 있는 부가 기능(예: 트랜잭션, 로깅 , 보안)을
//핵심 비즈니스 로직에서 떼어내, 한곳에 모아 관리하는 프로그래밍 방법.
//- 핵심 기능: 그 객체가 진짜 해야 하는 일(예: 레벨 업그레이드 규칙)
//- 부가 기능: 여러 곳에서 똑같이 필요한 보조 기능(예: 트랜잭션 경계)

//* 문제점
//5장에서 트랜잭션이라는 부가기능을 비즈니스 로직에서 분리함.
//-userServiceImple : 핵심 기능
//-UserServiceTx : 부가 기능 -> 손으로 만든 프록시

//프록시(proxy): 핵심기능을 감싸 부가 기능을 더하는 중간 오브젝트
//한계 : 부가 기능 적용 서비스가 늘어날 때마다, UserServiceTx 같은 프록시 클래스를
//일일이 만들어야 함.
//트랜잭션이 필요 없는 메서드까지 단순 위임 코드를 작성해야 함.

//=> 스프링이 제공하는 '다이나믹 프록시'로 해결
