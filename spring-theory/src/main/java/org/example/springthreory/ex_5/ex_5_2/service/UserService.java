package org.example.springthreory.ex_5.ex_5_2.service;

import org.example.springthreory.ex_5.ex_5_2.dao.Level;
import org.example.springthreory.ex_5.ex_5_2.dao.UserDAO;
import org.example.springthreory.ex_5.ex_5_2.domain.User;
...
public class UserService {
    ...
    // 업그레이드 담당
    //여러 사용자 업그레이드를 하나의 트랜잭션으로 묶는다
    //트랜잭션 경계 설정 : 트랜잭션의 시작을 선언하고, commit 또는 rollback으로 트랜잭션을 종료하는 작업
    public void upgradeLevels() throws SQLException, ClassNotFoundException {
        //1) 트랜잭션 시작(어떤 기술(jdbc, jpa, 분산 트랜잭션...)인지 모른 채, 추상화된 매니저에 맡긴다)
        ...
        try{
         //2) 비즈니스 로직 수행(이 안의 모든 update가 같은 트랜잭션에 묶인다
            ...
            //4)중간에 하나라고 실패하면 전부 취소(Rollback) -> 원자성 보장
