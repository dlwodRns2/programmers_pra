package org.example.springthreory.ch02.ex_2_1;

import org.example.springthreory.ch02.ex_2_1.dao.DaoFactory;
import org.example.springthreory.ch02.ex_2_1.dao.UserDAO;
import org.example.springthreory.ch02.ex_2_1.domain.User;
...
//Start.java 테스트의 문제점
//수동 확인 작업의 번거로움
//매번 Start.java를 실행하며 확인하는 실행 작업의 번거로움
...
//관심사의 분리라는 원리가 여기도 적용
//단위 테스트를 하는 이유는 개발자가 설계하고 만든 코드가 원래 의도한대로 동작하는지를
//개발자 스스로 빨리 확인받기 위함

//* 자동수행 테스트 코드
//테스트는 자동으로 수행되도록 코드로 만들어지는 것이 중요
//애플리케이션을 구성하는 클래스 안에 테스트 코드를 포함시키는 것보다는
//별도로 테스트용 클래스를 만들어 테스트 코드를 넣는게 좋다
//자동으로 수행되는 테스트의 장점은 자주 반복될 수 있다는 것이다.

//* 테스트의 결과
//모든 테스트는 성공/실패 두 가지 결과를 가질 수 있음
//또 테스트의 실패는 테스트가 진행되는 동안 에러가 발생해서 실패한 경우와,
//테스트 작업 중에 에러가 발생하진 않았지만, 그 결과가 기대값과 다른 경우로 구분

//JUnit
//JUnit은 자바에서 단위 테스트를 자동으로 작성,실행하게 해주는 표준 테스트 프레임워크
//위 Start.java처럼 main으로 직접 실행하고 콘솔을 눈으로 확인하던 방식을 대체
