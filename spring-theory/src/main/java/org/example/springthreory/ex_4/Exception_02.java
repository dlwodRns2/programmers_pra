package org.example.springthreory.ex_4;

//* 예외종류와 특징
//자바에서 throw 할 수 있는 모든 것은 Throwable의 자손(Throwable이 예외에 대한 모든 클래스의 조상)
//   Throwable
...
public class Exception_02 {
    ...
    void errorExam(){
        //아래처럼 끝없는 재귀를 호출하면 StackOverFlowError(Error의 자손)가 발생
        //errorExam();
        // => 이런 에러는 개발자가 try-catch로 다루는 대상이 아님. 그냥 코드를 고쳐야 하는 부분
    }
    ...
    void readFile() throws IOException{ // <- Checked Exception : throws로 선언하지 않으면 컴파일 안 됨
    ...
    void unCheckedExam(){ //<- UnChecked Exception : throws로 선언이 없어도 됨
        String str = null;
        str.length(); // -> NullPointerException(언체크 예외). null 체크를 안 한 "버그"다.
        setAge(-1);
    }
    void setAge(int age){
        if(age<0){
            throw new IllegalArgumentException("나이는 0 이상이여야한다.");
        }
    }
